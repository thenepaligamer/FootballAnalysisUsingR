...
rudiger <- fb_player_scouting_report("https://fbref.com/en/players/18b896d6/Antonio-Rudiger")

print(rudiger$Statistic)

footballer_selected <- rudiger[c(27,29,31,43,44, 47, 96, 87, 106, 102,108, 110, 122, 126, 149),]

footballer_selected

footballer_selected <- footballer_selected %>% 
  mutate(stat=case_when(Statistic == "Passes Completed" |
...
ggplot(footballer_selected,aes(fct_reorder(Statistic,stat),Percentile)) +  
  ...
  labs(fill="",   
       ...
       title=glue("{footballer_selected$Player[1]} | Chelsea FC"),
...
#ggsave("rudiger-pizzachart.png", width = 3000, height = 3000, units = "px")
